...
public class CommunicationsDataFactory {
	...
	{	
	    ...
	    try 
	    {
	    	...
	    	if(resultOwner!=null)
	    	{
	    		...
	    		try
	    		{
	    			 Date_Min = sdf.parse("12/26/2017");	//release start date 
	    			 Date_Max = sdf.parse("11/1/2018");	    //release end date
	    		}
				...
				for(int i=0;i<resultList.size();i++)
				 {
					...
					if((!tempList4.get(1).contains("Sprint 8"))||(tempList4.get(1).contains("Sprint 8.4"))) 
	...
	{	
		...
		Communications_PM_Data_Weekly_Trend.colorList=Arrays.asList(ColorFormater.RGB2String(145,38,41),ColorFormater.RGB2String(129,173,81),ColorFormater.RGB2String(58,63,113),
		...
		Communications_PM_Data_Weekly_Trend.tableData.addColumn(new ColumnDescription("y5", ValueType.INT, "Forecasted Trajectory"));
	...
	{	
		...
		Communications_PM_Data_Weekly_BurnDown.ticks=new ArrayList<>();
		...
		Communications_PM_Data_Weekly_BurnDown.tableData.addColumn(new ColumnDescription("x", ValueType.DATE, "Time"));
		...
		for(int i=0;i<dataCount;i++)
		{
		    ...
		    if(i%3==0)
		    {
		    	Communications_PM_Data_Weekly_BurnDown.ticks.add(ColorFormater.Str2JSStr(x_data.get(i)));
		    }
		}
		try 
		{
	...
	//	Communications_PM_Data_Feature_Progress.yTitle="Story Point";
		...
		Communications_PM_Data_Feature_Progress.chartLeft=7*length;
